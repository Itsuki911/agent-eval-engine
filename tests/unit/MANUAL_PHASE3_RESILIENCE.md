...
`pytest -s`で表示される確認結果は1行JSONである。`test`はテスト名、`result`は`passed`、`error_category`・`status_code`・`retry_count`・`llm_cost_usd`などのキーで判定する。下記の期待結果にある表示文は、対応するJSONのキー・値で確認する。APIキーは出力しない。

LLM呼び出しにおける各種エラー分類、上限超過、再試行、スキーマ検証、冪等性制御の例外処理を手動確認する単体テストケースです。
