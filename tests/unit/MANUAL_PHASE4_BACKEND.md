...
```powershell
docker compose --profile engine run --rm engine pytest -v -s tests/unit/test_tui_backend.py -k list_benchmarks
```

1. 実行コマンドを実行する。
2. テスト名と結果を確認する。

期待結果: 境界値。返される候補の`family`は`generic`または`coding`だけである。各候補の`path`は`benchmarks/`から始まり、terminal、web、guiのbenchmarkは候補へ含まれない。
