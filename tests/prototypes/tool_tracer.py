# 注意点: 引数のJSON直列化不可値はstr変換
# 別案: デコレータ形式のツール計測も併用可能
...
# 属性値を安全に変換する
def _sanitize_attribute(value: Any) -> str | int | float | bool:
    ...
    with tracer.start_as_current_span(span_name) as span:
        ...
        if arguments:
            for key, val in arguments.items():
                span.set_attribute(f"tool.arguments.{key}", _sanitize_attribute(val))
        try:
            yield span
