...
- 大量データセットに対応するため、CPU・メモリ使用量を制御・最適化する。必要に応じてRustを用いるシステムプログラミング層を導入し、OSの安全境界を守りながらハードウェア資源の利用効率を高める。
- データセットのストリーミング読み込みを実装し、全件をメモリへ保持せずに評価候補と評価データを処理できるようにする。
- データセット形式を見直し、メタデータと評価本文を分離するなどして、読み込み量・保存量・転送量を軽量化する。
- 評価エンジン側の処理を最小化し、不要な全件検証・全件転送・全件描画を避ける。
- LLM API呼び出しを非同期化し、並行実行、進捗表示、キャンセル、レート制限制御を行いながら高速なアクセスを実現する。
